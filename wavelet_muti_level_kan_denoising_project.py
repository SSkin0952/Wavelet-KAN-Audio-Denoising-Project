...
class Config:
    ...
    hidden_basis: int = 49             # number of basis functions in KAN-style shrinker
...
class WaveletCoeffLevelDataset(Dataset):
    """
    Dataset for one specific SWT detail level only.
    Input: noisy detail coefficients at level k
    Target: clean detail coefficients at level k
    """
    def __init__(self,
                 segments: np.ndarray,
                 wavelet: str,
                 levels: int,
                 level_idx: int,
                 snr_choices: Tuple[int, ...],
                 max_coeff_samples: int = 100000):
        """
        level_idx: 0-based index
            0 -> first SWT level returned by pywt.swt
            1 -> second level
            ...
        """
        xs = []
        ...
        print(f"Preparing level-{level_idx + 1} coefficient dataset from {len(segments)} segments...")
        ...
        for seg_idx, clean_seg in enumerate(segments):
            ...
            _, clean_d = clean_coeffs[level_idx]
            _, noisy_d = noisy_coeffs[level_idx]

            xs.append(noisy_d.reshape(-1, 1))
            ys.append(clean_d.reshape(-1, 1))

            if seg_idx % 20 == 0:
                print(f"Level {level_idx + 1} | Processed segments: {seg_idx + 1}/{len(segments)}")

        x_all = np.concatenate(xs, axis=0).astype(np.float32)
        y_all = np.concatenate(ys, axis=0).astype(np.float32)

        print(f"Level {level_idx + 1} | Total coefficient samples before subsampling: {len(x_all)}")

        if len(x_all) > max_coeff_samples:
            idx = np.random.choice(len(x_all), size=max_coeff_samples, replace=False)
            x_all = x_all[idx]
            y_all = y_all[idx]
            print(f"Level {level_idx + 1} | Subsampled coefficient samples: {len(x_all)}")
        ...
        print(f"Level {level_idx + 1} | Final dataset size: {self.x.shape}")
...
class KANShrinkage1D(nn.Module):
    def __init__(self, n_basis: int = 33, init_scale: float = 0.35):
        super().__init__()
        centers = torch.linspace(-3.0, 3.0, n_basis).view(1, n_basis)
        self.centers = nn.Parameter(centers)
        self.log_scales = nn.Parameter(torch.full((1, n_basis), math.log(init_scale)))
        self.weights = nn.Parameter(torch.zeros(1, n_basis))
        self.linear_w = nn.Parameter(torch.ones(1, 1))
        self.linear_b = nn.Parameter(torch.zeros(1, 1))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: [B, 1]
        z = x - self.centers
        scales = torch.exp(self.log_scales).clamp_min(1e-4)
        phi = torch.exp(-0.5 * (z / scales) ** 2)     # gaussian basis
        nonlinear = phi @ self.weights.t()            # [B, 1]
        linear = x @ self.linear_w + self.linear_b
...
def learned_denoise_multi_level(noisy: np.ndarray,
                                models: List[nn.Module],
                                norm_mus: List[torch.Tensor],
                                norm_stds: List[torch.Tensor],
                                wavelet: str,
                                levels: int,
                                device: str) -> np.ndarray:
    """
    Apply one learned shrinkage model per SWT detail level.
    """
    coeffs, original_len = swt_decompose(noisy, wavelet, levels)
    new_coeffs = []
    ...
    with torch.no_grad():
        for level_idx, (cA, cD) in enumerate(coeffs):
            ...
            norm_mu = norm_mus[level_idx]
            norm_std = norm_stds[level_idx]
...
def evaluate_method(segments: np.ndarray,
                    ...
                    norm_mus: List[torch.Tensor] = None,
                    norm_stds: List[torch.Tensor] = None) -> Dict[int, Dict[str, float]]:
    results = {}
    ...
    for snr in cfg.snr_eval:
        ...
        for clean_seg in segments:
            ...
            elif method_name == "learned_multi":
                est = learned_denoise_multi_level(
                    noisy_seg,
                    models,
                    norm_mus,
                    norm_stds,
...
def plot_history(history: Dict[str, List[float]], out_path: str, title: str) -> None:
...
def plot_shrinkage_curve(model: nn.Module, out_path: str, device: str, title: str) -> None:
...
def run(cfg: Config) -> None:
    ...
    print("Building per-level coefficient datasets and training per-level models...")
    ...
    for level_idx in range(cfg.levels):
        print(f"\n===== Level {level_idx + 1}/{cfg.levels} =====")

        train_ds_level = WaveletCoeffLevelDataset(
            train_seg,
            cfg.wavelet,
            cfg.levels,
            level_idx,
            cfg.snr_train,
            cfg.max_coeff_samples,
        )

        val_ds_level = WaveletCoeffLevelDataset(
            val_seg,
            cfg.wavelet,
            cfg.levels,
            level_idx,
            cfg.snr_train,
            min(10000, cfg.max_coeff_samples),
        )

        train_loader_level = DataLoader(
            train_ds_level,
            batch_size=cfg.batch_size,
            shuffle=True,
            num_workers=cfg.num_workers,
        )
        val_loader_level = DataLoader(
            val_ds_level,
            batch_size=cfg.batch_size,
            shuffle=False,
            num_workers=cfg.num_workers,
        ...
        print(f"Training learned shrinkage model for level {level_idx + 1}...")
        model_level = KANShrinkage1D(
            n_basis=cfg.hidden_basis,
            init_scale=cfg.basis_scale,
        )

        model_level, history_level = train_model(
            model_level,
            train_loader_level,
            val_loader_level,
            cfg,
        ...
        histories.append(history_level)
        norm_mus.append(train_ds_level.mu)
        norm_stds.append(train_ds_level.std)

    # save model and normalizer from training dataset
    for level_idx in range(cfg.levels):
        ckpt = {
            "model_state": models[level_idx].state_dict(),
            "norm_mu": norm_mus[level_idx],
            "norm_std": norm_stds[level_idx],
            "level_idx": level_idx,
            "config": cfg.__dict__,
        ...
        torch.save(
            ckpt,
            os.path.join(cfg.out_dir, f"learned_shrinkage_model_level{level_idx + 1}.pt")
        )
    ...
    save_results_table(results_all, os.path.join(cfg.out_dir, "results_multilevel.csv"))

    print("Saving plots and demo audio...")
    for level_idx in range(cfg.levels):
        plot_history(
            histories[level_idx],
            os.path.join(cfg.out_dir, f"training_history_level{level_idx + 1}.png"),
            title=f"Training history - Level {level_idx + 1}"
        )
        plot_shrinkage_curve(
            models[level_idx],
            os.path.join(cfg.out_dir, f"learned_shrinkage_curve_level{level_idx + 1}.png"),
            cfg.device,
            title=f"Learned shrinkage function - Level {level_idx + 1}"
        )

    # Use a longer continuous demo clip for listening
    demo_len = int(cfg.demo_seconds * sr)

    if len(audio) <= demo_len:
        demo_clean = audio.copy()
    else:
        start_sample = len(audio) // 2
        if start_sample + demo_len > len(audio):
            start_sample = len(audio) - demo_len
        demo_clean = audio[start_sample:start_sample + demo_len].copy()
    ...
    for snr_demo in [0, 5, 10]:
        ...
        demo_learned = learned_denoise_multi_level(
            demo_noisy,
            models,
            norm_mus,
            norm_stds,
        ...
        sf.write(os.path.join(cfg.out_dir, "demo_clean.wav"), demo_clean, sr)
